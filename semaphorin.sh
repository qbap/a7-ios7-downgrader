...
_download_boot_files() {
    ...
    if [ ! -e "$dir"/$1/$cpid/$3/kernelcache ]; then
        ...
        if [[ "$3" == "7."* ]]; then
            ...
            "$bin"/iBoot64Patcher "$dir"/$1/$cpid/$3/iBSS.dec "$dir"/$1/$cpid/$3/iBSS.patched
            if [[ "$3" == "10.3"* ]]; then
                "$bin"/iBoot64Patcher "$dir"/$1/$cpid/$3/iBEC.dec "$dir"/$1/$cpid/$3/iBEC.patched -b "$boot_args rd=disk0s1s8 amfi=0xff cs_enforcement_disable=1 keepsyms=1 debug=0x2014e PE_i_can_has_debugger=1 amfi_get_out_of_my_way=1 amfi_allow_any_signature=1" -n
            else
        ...
        if [[ "$3" == "8.0" ]]; then
            ...
            "$bin"/seprmvr64 "$dir"/$1/$cpid/$3/kcache.raw "$dir"/$1/$cpid/$3/kcache.patched
            "$bin"/KPlooshFinder "$dir"/$1/$cpid/$3/kcache.patched "$dir"/$1/$cpid/$3/kcache2.patched
            "$bin"/Kernel64Patcher "$dir"/$1/$cpid/$3/kcache2.patched "$dir"/$1/$cpid/$3/kcache3.patched -a -f -m
...
if [[ "$ramdisk" == 1 || "$restore" == 1 || "$dump_blobs" == 1 || "$fix_activation" == 1 || "$dump_nand" == 1 || "$restore_nand" == 1 || "$restore_mnt1" == 1 || "$restore_mnt2" == 1 || "$disable_NoMoreSIGABRT" == 1 || "$NoMoreSIGABRT" == 1 ]]; then
    ...
    if [[ "$restore" == 1 ]]; then
        ...
        if [[ "$version" == "10.3"* || "$version" == "11."* || "$version" == "12."* ]]; then
            ...
            "$bin"/iBoot64Patcher "$dir"/$deviceid/$cpid/$version/iBEC.dec "$dir"/$deviceid/$cpid/$version/iBEC.patched -b "$boot_args rd=$systemfs amfi=0xff cs_enforcement_disable=1 keepsyms=1 debug=0x2014e PE_i_can_has_debugger=1 amfi_get_out_of_my_way=1 amfi_allow_any_signature=1" -n
        ...
        if [[ "$version" == "10.3"* || "$version" == "11."* || "$version" == "12."* ]]; then
            ...
            if [[ "$version" == "10."* ]]; then
                ...
                "$bin"/sshpass -p "alpine" scp -o StrictHostKeyChecking=no -P 2222 "$dir"/jb/Undecimus.app.tar root@localhost:/mnt4/
                "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost 'tar --preserve-permissions -xvf /mnt4/Undecimus.app.tar -C /mnt4/Applications' 2> /dev/null
                "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost 'rm -rf /mnt4/Undecimus.app.tar' 2> /dev/null
            fi
            ...
            #if [ -e "$dir"/$deviceid/0.0/Carrier_Bundles.tar ]; then
            #    "$bin"/sshpass -p "alpine" scp -o StrictHostKeyChecking=no -P 2222 "$dir"/$deviceid/0.0/Carrier_Bundles.tar root@localhost:/mnt1/Carrier_Bundles.tar 2> /dev/null
            #    "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "rm -rf /mnt1/System/Library/Carrier\ Bundles/iPhone/*" 2> /dev/null
            #    "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "tar -xvf /mnt1/Carrier_Bundles.tar -C /" 2> /dev/null
            #fi
            if [[ "$version" == "10."* ]]; then
