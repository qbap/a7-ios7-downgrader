...
if [[ "$ramdisk" == 1 || "$restore" == 1 || "$dump_blobs" == 1 ]]; then
    ...
    if [[ "$restore" == 1 ]]; then
        ...
        if [[ "$version" == "10.3"* || "$version" == "11."* || "$version" == "12."* ]]; then
            ...
            remote_cmd "/sbin/newfs_apfs -A -v SystemA /dev/disk0s1" && {
                sleep 1
            } || {
                "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "/sbin/newfs_apfs -A -v SystemH /dev/disk0s1"
            }
            sleep 2
            remote_cmd "/sbin/apfs_deletefs /dev/$systemfs" && {
                sleep 1
                remote_cmd "/sbin/newfs_apfs -A -v SystemB /dev/disk0s1" && {
                    sleep 1
                } || {
                    "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "/sbin/newfs_apfs -A -v SystemI /dev/disk0s1"
                }
                sleep 2
            } || {
                remote_cmd "/sbin/apfs_deletefs /dev/$systemfs" && {
        ...
        if [[ "$version" == "7."* || "$version" == "8."* || "$version" == "9."* || "$version" == "10.0"* || "$version" == "10.1"* || "$version" == "10.2"* ]]; then
            ...
            "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "/sbin/umount /mnt4" 2> /dev/null
            "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "/sbin/umount /mnt5" 2> /dev/null
            "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "/sbin/mount_apfs /dev/disk0s1s8 /mnt4"
            "$bin"/sshpass -p 'alpine' ssh -o StrictHostKeyChecking=no -p2222 root@localhost "/sbin/mount_apfs /dev/disk0s1s9 /mnt5"
        fi
